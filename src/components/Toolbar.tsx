...
const Brand = memo(function Brand({ count }: { count: number }) {
  return (
    ...
    >
      <div className="grid h-7 w-7 place-items-center rounded-[8px] bg-ink-950 text-white shadow-soft dark:bg-white dark:text-ink-950">
      ...
      <div className="leading-none">
        <div className="flex items-baseline gap-1.5">
          <span className="text-[14px] font-semibold tracking-[-0.018em] lowercase text-ink-900 dark:text-ink-100">
            inklin
          </span>
          <span className="font-mono text-[9.5px] tracking-tight text-ink-400 dark:text-ink-500">
            /ˈɪŋklɪn/
          </span>
        </div>
        <div className="mt-1 font-mono text-[10px] tracking-tight uppercase text-ink-500 dark:text-ink-400">
...
function IconButton({
  label, onClick, primary, children, dataTour,
}: {
  label: string
  onClick: () => void
  primary?: boolean
  ...
  return (
    <motion.button
      ...
      className={[
        ...
        'dark:text-ink-300 dark:hover:text-ink-100',
        primary ? 'h-8 w-8 glass-btn' : 'h-8 w-8 hover:bg-black/[0.05] dark:hover:bg-white/[0.06]',
      ].join(' ')}
    >
      {children}
    </motion.button>
...
export default function Toolbar({
  scale, count, soundEnabled, theme, dimmed,
  onZoomIn, onZoomOut, onResetView,
  onAdd, onToggleSound, onToggleTheme, onReplayTour, playSound,
}: Props) {
  ...
  return (
    <>
      ...
      >
        <div className="glass flex items-center gap-0.5 rounded-2xl px-1.5 py-1.5">
          ...
          <button
            ...
            className="min-w-[3.25rem] rounded-md px-1.5 py-1 font-mono text-[11px] tabular-nums tracking-tight text-ink-700 hover:text-ink-900 hover:bg-black/[0.05] transition-colors dark:text-ink-300 dark:hover:text-ink-100 dark:hover:bg-white/[0.06]"
          >
          ...
          <IconButton
            label="Replay tour"
            onClick={wrap(onReplayTour, 'tapSoft')}
          >
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
          ...
          <IconButton
            ...
            onClick={wrap(onToggleTheme, 'tapSoft')}
          >
            {theme === 'dark' ? (
              // Sun
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      ...
      >
        <div className="flex items-center gap-2 font-mono text-[10.5px] tracking-tight text-ink-400 dark:text-ink-500">
...
function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="glass-btn rounded-[5px] px-1.5 py-0.5 font-mono text-[10px] tracking-tight text-ink-700 dark:text-ink-300">
      {children}
    </kbd>
  )
}
